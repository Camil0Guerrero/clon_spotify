...
export default function button(playerButton, d = document) {
	// The player button is necessary because it contains the id of the song
	const $audio = d.getElementById(playerButton),
	...
	const changeTimeLinePosition = () => {
		...
		d.querySelector(".current-time").innerHTML = (
			"0" + `${~~$audio.currentTime}`
		).slice(-2);
	};
